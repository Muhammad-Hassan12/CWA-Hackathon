...
async def get_procedure(procedure_id: str):
    ...
    try:
        ...
        logger.error(f"Error fetching procedure {procedure_id}: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def report_exploitation_signal(signal: ExploitationSignalCreate):
    ...
    try:
        ...
        logger.error(f"Failed to record exploitation signal: {e}")
        raise HTTPException(status_code=500, detail="Failed to persist exploitation signal")
